...
def _get_nim_models():
    resp = httpx.get(
        f"{config.NVIDIA_NIM_BASE_URL}/models",
    ...
    for m in models:
        ...
        context_length = m.get("context_length") or 131072
        supports_vision = "vision" in model_id.lower() or "vl" in model_id.lower()
        result.append({
            "id": f"nvidia-nim/{model_id}",
            "name": f"{name} (NIM)",
...
def get_models():
    ...
    if config.NVIDIA_NIM_API_KEY:
        try:
            nim_models = _get_nim_models()
        except Exception:
            nim_models = []
        parsed = nim_models + parsed
